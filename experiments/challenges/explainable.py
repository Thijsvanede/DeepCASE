...
if __name__ == "__main__":
    ...
    # Load data
    _, encodings = PreprocessLoader().load(args.file, args.dim_input, args.time, args.dim_output,
        max_events    = args.max_events,
        max_sequences = args.max_sequences,
        offset        = args.offset,
        train_ratio   = 0,
        random        = args.random,
        extract       = ['ts_start', 'threat_name'],
    ...
    cluster = Cluster2(p=1)

    for label, tree in deepseq.interpreter.tree.items():
        scores = deepseq.interpreter.labels[label]
        data, index, nodes, bounds = tree.get_arrays()
        clusters = cluster.dbscan(data,
            eps         = 0.1,
            min_samples = 1,
            verbose     = False,
        )
        for c, indices in lookup_table(clusters):
            score = np.asarray([scores.get(i) for i in index[indices]])
            ...
            if score[1] > 0:
                ...
                score = 'HIGH'
            print(cluster_summary(label, data[indices], score, indices.shape[0], decoding))
        print()
