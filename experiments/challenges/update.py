...
if __name__ == "__main__":
    ...
    deepseq_orig.context_builder = ContextBuilder.load(
        infile = '../../models/context/company.save',
        device = args.device,
    )

    deepseq_orig.interpreter = Interpreter.load(
        infile          = 'interpreter_max.save',
        context_builder = deepseq_orig.context_builder,
    )
    ...
    while time_test < time_end:
        ...
        ################################################################
        #              Create updated version of DeepSeq               #
        ################################################################

        deepseq_update = DeepSeq(
            n_features  = args.features,
            complexity  = args.complexity,
            context     = args.dim_input,
            device      = args.device,
        ...
        deepseq_update.context_builder = ContextBuilder.load(
            infile = '../../models/context/company_{}m.save'.format(month+1),
            device = args.device,
        )
        ...
        deepseq_update.interpreter.fit(X_train, y_train,
        ...
        ################################################################
        # Predict test data
        result_test_orig = deepseq_orig.interpreter.predict(
            X_test,
            y_test,
            verbose = args.verbose,
        )

        result_test_update = deepseq_update.interpreter.predict(
            X_test,
            y_test,
            verbose = args.verbose,
        )
        ...
        y_pred_test_update = np.zeros(result_test_update.shape[0], dtype=int)
        y_pred_test_orig  [:] = result_test_orig  [:, 0]
        y_pred_test_update[:] = result_test_update[:, 0]
        ...
        # Set impact - train
        y_pred_test_update[result_test_update[:, 0] >=  0] = 0
        y_pred_test_update[result_test_update[:, 0] >=  5] = 1
        y_pred_test_update[result_test_update[:, 0] >= 30] = 2
        y_pred_test_update[result_test_update[:, 0] >= 70] = 3
        # Set breaches
        y_pred_test_update[result_test_update[:, 1] > 0] = 4
        ...
        header("Statistics - Workload Reduction")
        datapoints = y_pred_test_update.shape[0]
        automated  = (y_pred_test_update >= 0).sum()
        anomalous  = (y_pred_test_update <  0).sum()
        anomalies_conf  = (y_pred_test_update == -1).sum()
        anomalies_train = (y_pred_test_update == -2).sum()
        anomalies_eps   = (y_pred_test_update == -3).sum()
        width = len(str(datapoints))
        print("Datapoints            : {:{width}}".format(datapoints, width=width))
        print("Automated             : {:{width}}/{:{width}} = {:6.2f}%".format(automated, datapoints, 100*automated/datapoints, width=width))
        print("Anomalous             : {:{width}}/{:{width}} = {:6.2f}%".format(anomalous, datapoints, 100*anomalous/datapoints, width=width))
        ...
        header("Statistics - Anomalies")
        y_true_anomalous = y_true_test       [y_pred_test_update < 0]
        y_pred_anomalous = y_pred_test_update[y_pred_test_update < 0]
        ...
        header("Performance")
        mask = y_pred_test_update >= 0
        print(classification_report(
            y_true        = y_true_test[mask],
            y_pred        = y_pred_test_update[mask],
            digits        = 4,
            labels        = [0, 1, 2, 3, 4],
            target_names  = ['INFO', 'LOW', 'MEDIUM', 'HIGH', 'ATTACK'],
            zero_division = 0,
