...
if __name__ == "__main__":
    ...
    X_train = X[mask_train].to(args.device)[:, -10:]
    X_test  = X[mask_test ].to(args.device)[:, -10:]
    ...
    for iterations in [100, 0]:
        ...
        # Get scores
        score = torch.stack((impact_train, breach_train), dim=2)
        ...
        # Predict train data
        result = deepseq.interpreter.predict(X_train, y_train, verbose=args.verbose, iterations=iterations)

        mask = result[:, 0] >= 0
        result_ = result[mask]
        impact_ = impact_test[mask]
        breach_ = breach_test[mask]

        y_pred = np.zeros(result_.shape[0])
        y_pred[result_[:, 0] >   0] = 1
        y_pred[result_[:, 0] >= 30] = 2
        y_pred[result_[:, 0] >= 70] = 3
        y_pred[result_[:, 1] >   0] = 4

        y_true = np.zeros(result_.shape[0])
        y_true[impact_.squeeze(1) >   0] = 1
        y_true[impact_.squeeze(1) >= 30] = 2
        y_true[impact_.squeeze(1) >= 70] = 3
        y_true[breach_.squeeze(1) >   0] = 4

        print(classification_report(y_true, y_pred, digits=4))
        print((result[:, 0] < 0).sum())
