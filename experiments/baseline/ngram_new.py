...
if __name__ == "__main__":
    ...
    # Load data from file
    events, context, label, mapping = preprocessor.csv(args.file)
    # Set to device
    events  = events .to(args.device).unsqueeze(1)
    context = context.to(args.device)
    label   = label  .to(args.device)

    X = torch.cat((context, events), dim=1)
    y = label

    X_train = X[:int(args.train) ]
    y_train = y[:int(args.train) ]
    X_test  = X[ int(args.train):]
    y_test  = y[ int(args.train):]
